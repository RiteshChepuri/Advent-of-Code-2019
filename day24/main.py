...
def count_bugs(x, y):
    ...
    return sum(
        1 for x_coord, y_coord in coords if grid.get((x_coord, y_coord), ".") == "#"
    )
...
def count_bugs_rec(x, y, n):
    ...
    for elem in coords_remove:
        coords.remove(elem)
    coords = list(set(coords))
    return sum(
        1 for x_coord, y_coord, dim in coords if get_dim(dim)[x_coord, y_coord] == "#"
    )
